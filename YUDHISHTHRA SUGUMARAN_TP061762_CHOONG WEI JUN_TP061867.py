...
# Function to format prompt message to accept input from users
def userInput(promptMessage: str, skipLine: bool) -> input:
    if (skipLine):
        return input(f"\n{promptMessage} >> ")
    else:
...
# Verifies if username exists in the respective list
def authUsername(username: str, detailsList: list) -> bool:
    for data in detailsList:
        if username == data[0]:
            return True


# Verifies if password exists in the respective list
def authPassword(username: str, password: str, detailsList: list) -> bool:
    for data in detailsList:
        if username == data[0] and password == data[1]:
            return True


# Skips n number of lines in a file handle (textIOWrapper) that is being read/write
def skipFileLine(count: int, fileHandle):
    for _ in range(count):
        next(fileHandle)


# Removes list with empty strings and lists with no elements
def removeEmptyList(sourceList: list) -> list:
...
# Displays banner for each different pages for respective dashboards
def pageBanners(pageTitle: str, centerLength: int):
...
def adminMenu(uName: str = ""):  # Admin Menu shown upon successful login
    while True:
        try:
            ...
            print("1. Add food item", "2. Modify food item",
                  "3. Display records", "4. Search record", "0. Log out", sep='\n')
...
def addFoodItemMenu():  # Prompts admin to select which category of food item they want to add or to add new food category
    while True:
        try:
            ...
            chosenFoodCategoryNumber = int(
                userInput("Food Category Number", True))
            if (chosenFoodCategoryNumber == 0):
                ...
                writeNewFoodCategoryToFile(
                    newFoodCategoryName, newFoodCategoryDescription)
                print(f"Would you like to add a new food item in this category?")
                userConfirmation = userInput("(Y)es/(N)o", False).upper()
                if userConfirmation == 'Y':
                    foodItemName, foodItemPrice = getNewFoodItemDetails(
                        newFoodCategoryName)
                    writeNewFoodItemToFile(
                        newFoodCategoryName, foodItemName, foodItemPrice)
                else:
                    addFoodItemMenu()
            elif (chosenFoodCategoryNumber > 0 and chosenFoodCategoryNumber <= len(foodCategoryTitles)):
                chosenFoodCategoryName = foodCategoryTitles[int(
                    chosenFoodCategoryNumber)-1][0].replace("FOOD CATEGORY", "").strip().capitalize()
                foodItemName, foodItemPrice = getNewFoodItemDetails(
                    chosenFoodCategoryName)
                print("\nPlease confirm the new food details you would like to add")
                userConfirmation = userInput("(Y)es/(N)o", False).upper()
                if (userConfirmation == 'Y'):
                    writeNewFoodItemToFile(
                        chosenFoodCategoryName, foodItemName, foodItemPrice)
...
# Gets details of new food item details from user
def getNewFoodItemDetails(chosenFoodCategoryName: str) -> str:
    try:
        foodItemName = userInput("New Food Item Name", False)
        foodItemPrice = format(
            float(userInput("New Food Item Price", False)), ".2f")
        print(
            f"\nFOOD CATEGORY\tFOOD ITEM PRICE\t FOOD ITEM NAME\n{'-'*13}{' '*3}{'-'*15}{' '*2}{'-'*14}")
        print('{:<16}{:<17}{}'.format(
            chosenFoodCategoryName, foodItemPrice, foodItemName))
        return foodItemName, foodItemPrice
    except ValueError:
        print("Invalid value entered")


# Adds new food category in food details file
def writeNewFoodCategoryToFile(categoryName: str, categoryDescription: str):
    ...
    with open(FOOD_DETAILS_FILE, 'a') as foodDetailsFile:
        ...
        foodDetailsFile.write(
            f"{uppercaseName} FOOD CATEGORY - {capitalizedDescription}\n")
        foodDetailsFile.write("_"*88)


# Directly writes new food item data to food details file
def writeNewFoodItemToFile(foodCategoryName: str, foodItemName: str, foodItemPrice: float):
    try:
        orderRecordsList = []
        ...
        if not foodItemIndexes:
            ...
            lastFoodItemRecord = orderRecordsList[lastFoodItemIndex][0].split(
                " | ")
            newFoodID = lastFoodItemRecord[0][0] + str(
                int(lastFoodItemRecord[1].strip(lastFoodItemRecord[0][0]))+1)
        if (lastFoodItemIndex == (len(orderRecordsList)-1)):
            orderRecordsList.insert(
                lastFoodItemIndex+1, [f'\n{foodCategoryName} | {newFoodID} | {foodItemName} | {foodItemPrice}'])
        else:
            orderRecordsList.insert(
                lastFoodItemIndex+1, [f'{foodCategoryName} | {newFoodID} | {foodItemName} | {foodItemPrice}\n'])
...
# Verifies the format and existence of the food item ID that has been received by the user
def verifyFoodItemId(foodItemId: str, foodDetailsList: list) -> bool:
    if foodItemId == '':
        return False
...
def listOutFoodItems(chosenFoodCategoryName: str):
    while True:
        try:
            time.sleep(0.5)
...
# Menu for user to choose which specific food item id they want to update
def updateFoodItemMenu(foodDetailsList):
    while True:
        try:
            foodCategoryTitles = extractFoodCategories()
            ...
            if 0 < categoryNumber <= len(foodCategoryTitles):
                ...
                foodItemId = userInput(
                    "Enter the Food Item ID that you would like to update", True).upper()
                if (verifyFoodItemId(foodItemId, foodDetailsList)):
                    ...
                    updateFoodItemRecord(
                        foodDetailsList, foodItemIdList, foodItemIdIndex)
                    break
                else:
...
# Menu for user to choose which specific food item id they want to delete
def deleteFoodItemMenu(foodDetailsList):
    while True:
        try:
            foodCategoryTitles = extractFoodCategories()
            ...
            if 0 < categoryNumber <= len(foodCategoryTitles):
                ...
                listOutFoodItems(foodCategoryTitles[categoryNumber-1][0])
                foodItemId = userInput(
                    "Enter the Food Item ID that you would like to delete", True).upper()
                if(verifyFoodItemId(foodItemId, foodDetailsList)):
                    ...
                    if (userConfirmation in ('Y', 'N')):
                        deleteFoodItemRecord(
                            userConfirmation, foodItemId, foodDetailsList)
                        break
                    else:
                        print("Invalid input")
                else:
...
# Updates a specific record of food item in the food details text file
def updateFoodItemRecord(foodDetailsList, foodItemIdList, foodItemIdIndex):
    try:
        print("\nWhat would you like to update?", "1. Food Item Price",
              "2. Food Item Name", "3. Both", sep='\n')
        while True:
...
# Deletes a specific food item record in the food details text file
def deleteFoodItemRecord(userConfirmation, foodItemId, foodDetailsList):
    while True:
        try:
            if (userConfirmation == "Y"):
                for data in foodDetailsList:
        ...
        for list in removeEmptyList(rawList):
            for data in list:
                for i in range(len(data)):
                    if data[i] == "-":
                        foodCategoryDetails.append(
                            [''.join(data[0:i-1]), ''.join(data[i+2:-1])])
...
def displayRecordsMenu():  # Display records main page
    print("\n1. Food Categories", "2. Food Items by Category", "3. Customer Orders",
          "4. Customer Payment", "0. Back to Admin  Menu", sep='\n')
    while True:
        try:
...
# Displays the records of food categories and descriptions cleanly
...
# Displays either order or payment records based on parameters given
...
# Search customer order by username
def searchOrderByUsername(orderRecordsList: list):
    try:
        recordByUsername = []
...
# Search customer order by Username
def searchPaymentByUsername(paymentList: list):
    try:
        recordByUsername = []
        username = userInput("Please enter Customer Username", True)
        count = 0
...
def searchPaymentById(paymentList: list):  # Search customer order by Order ID
    ...
    for data in paymentList:
        if (orderID.upper() == data[1]):
            recordById.append([data[0], data[1], data[3],
                              data[4], data[5], data[6]])
            orderIdExists = True
    if (orderIdExists):
        print("1 payment record have been found for Order ID {}".format(
            orderID.upper()))
        progressBar("Generating report")
        time.sleep(0.5)
...
# Gets search results from the earlier search functions and displays it cleanly
def displaySearchResults(recordName: str, searchBasis: str, resultsList: list):
    if recordName == 'orders':
        ...
        for data in resultsList:
            print('{:<24}{:<16}{:<16}{:<16}{:<16}{}'.format(
                data[0].upper(), data[1], data[2], data[3], data[4], data[5]))
    ...
    while True:
        ...
        try:
            ...
            if chosenCategory == 0:
                ...
                listOutFoodItemsNoDetails(
                    (extractFoodCategories()[chosenCategory-1][0]))
            else:
                print("\nNumber out of range!")
                time.sleep(1.5)
                viewCategoryList()
    ...
    while True:
        try:
            ...
            if chosenCategory == "0":
                ...
                cart = itemEngine(cart, (extractFoodCategories()[int(chosenCategory)-1][0]))
            else:
...
def removeItemFromCart(item: str, amount: str, cart: list):
    if amount.lower() != "all":
        try:
            ...
            input(
                "Oops! Invalid string found, do you mean \"All\"?\nPress enter to return.")
            return cart
    for i in range(0, len(cart), 2):
        if cart[i] == item:
...
def main():  # The main module that will be executed first
    while True:
        ...
        print("\nWho are you logging in as?", "1. Admin",
              "2. Customer", "3. Quit Program", sep=' \n')
        try:
